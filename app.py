...
with onglet_respiration:
    ...
    # -------------------- BOUTON ------------------------
    # =====================================================
    start = st.button("▶️ Démarrer l'exercice")
    ...
    if start:
        ...
        function unlockAudio() {{
            if (!audioUnlocked && audioOn) {{
                sndInspire.play().then(() => {{
                    sndInspire.pause();
        ...
        function playSound(name) {{
            ...
            sndInspire.pause(); sndRetiens.pause(); sndExpire.pause();
            if (name==="inspire"){{sndInspire.currentTime=0;sndInspire.play();}}
            if (name==="retenue"){{sndRetiens.currentTime=0;sndRetiens.play();}}
            if (name==="expire"){{sndExpire.currentTime=0;sndExpire.play();}}
        }}
        ...
        function show(text,sound){{
            ...
            setTimeout(()=>{{phaseText.innerText=text;phaseText.style.opacity=1;if(sound) playSound(sound);}},200);
        }}
        ...
        function animate(ts){{
            ...
            if(progress>=1){{
                if(phase==="inspire"){{phase="retenue";scaleFrom=1.4;scaleTo=1.4;if(retenue>0)show("Retiens","retenue");}}
                else if(phase==="retenue"){{phase="expire";scaleFrom=1.4;scaleTo=1;show("Expire","expire");}}
                else{{cycle++;if(cycle>=cycles){{show("Terminé",null);running=false;return;}}phase="inspire";scaleFrom=1;scaleTo=1.4;show("Inspire","inspire");}}
                startTime=ts;
            }}
        ...
        // ------------------ CONTROLES -------------------
        // =================================================
        function toggle(){{running=!running;unlockAudio();}}
        function stopAll(){{running=false;cercle.style.transform="scale(1)";phaseText.innerText="Arrêté";}}
